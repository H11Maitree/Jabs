(function (global, factory) {
  ...
  var Line = /*#__PURE__*/function () {
    function Line(_ref) {
      var index = _ref.index,
          ...
          y2 = _ref.y2,
          group = _ref.group;
      this.shape = map.canvas.createLine({
        x1: x1,
        y1: y1,
        x2: x2,
        y2: y2,
    ...
    if (animate && count > 0) {
      ...
      interval = setInterval(function () {
        ...
        if (i == count) {
          ...
          _this.emit(Events.onViewportChange, [_this.scale, _this.transX, _this.transY]);
        }
      }, 10);
    } else {
      this.transX = transX;
      ...
      this.scale = scale;
      this.applyTransform();
      this.emit(Events.onViewportChange, [this.scale, this.transX, this.transY]);
    }
  }
  ...
  var Map = /*#__PURE__*/function () {
    ...
    _proto.init = function init(selector) {
      ...
      this.createMarkers(options.markers); // Position labels

      this.repositionLabels(); // Handle the container

      this.handleContainerEvents(); // Handle regions/markers events

      this.handleElementEvents(); // Create toolip
    ...
    _proto.destroy = function destroy(destroyInstance) {
      ...
      this.emit(Events.onDestroyed); // For perfomance issues remove all possible properties
